...
+ (NSString *)XMLStringForNode:(id)node withNodeName:(NSString *)nodeName
{
    if ([node isKindOfClass:[NSArray class]]) {
        ...
        for (id individualNode in node) {
            NSString *nodeName = [individualNode isKindOfClass:[NSDictionary class]]
                                     ? ([individualNode nodeName] ?: nodeName)
                                     : nodeName;
        ...
        for (NSString *key in [attributes allKeys]) {
            [attributeString appendFormat:@" %@=\"%@\"", [[key description] XMLEncodedString],
        ...
        if ([innerXML length]) {
            return [NSString
                stringWithFormat:@"<%1$@%2$@>%3$@</%1$@>", nodeName, attributeString, innerXML];
        } else {
            return [NSString stringWithFormat:@"<%@%@/>", nodeName, attributeString];
        }
    } else {
        return [NSString
            stringWithFormat:@"<%1$@>%2$@</%1$@>", nodeName, [[node description] XMLEncodedString]];
    }
}
...
{
    [self endText];
    //	elementName = _nodeNameMaps[elementName] ?:elementName;
...
+ (NSDictionary *)dictionaryWithXMLData:(NSData *)data nodeNameMaps:(NSDictionary *)nodeNamesMaps
{
    XMLDictionaryParser *parser = [[XMLDictionaryParser sharedInstance] copy];
    parser.nodeNameMaps = [nodeNamesMaps copy];
    ...
    if (attributes) {
        ...
        for (NSString *key in [filteredDict allKeys]) {
            ...
            if ([key hasPrefix:XMLDictionaryAttributePrefix]) {
                filteredDict[[key substringFromIndex:[XMLDictionaryAttributePrefix length]]] =
                    self[key];
            }
        }
...
+ (NSDictionary *)dictionaryByRemoveAttributesPrefixFromXMLDictionary:(NSDictionary *)object
{
    if ([object isKindOfClass:[NSArray class]]) {
        NSMutableArray *arr = [[NSMutableArray alloc] init];
        for (id obj in object) {
            [arr addObject:[self dictionaryByRemoveAttributesPrefixFromXMLDictionary:obj]];
        }
        return (id)arr;
    } else if ([object isKindOfClass:[NSDictionary class]]) {
        NSMutableDictionary *dict = [NSMutableDictionary dictionary];
        [dict addEntriesFromDictionary:(id)[object attributes]];
        [dict addEntriesFromDictionary:[object childNodes]];
        if (object[@"__text"]) {
            dict[@"__text"] = object[@"__text"];
        }
        NSMutableDictionary *resultDict = [NSMutableDictionary dictionary];
        [dict enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
            [resultDict setObject:[self dictionaryByRemoveAttributesPrefixFromXMLDictionary:obj] forKey:key];
        }];
        return resultDict;
    } else {
        return object;
